...
def test2(src_slc, dst_slc, n = 10):
    ...
    for i in xrange(n):
        ...
        f = range_filter(r)[dst_slc]
        info.append([
            r, f, s[f], r[f], np.sum(f),
        ])
    return s, info

def plot_history(src, n):
    n += 1
    sc = 4./3.
    ny = np.floor(np.sqrt(n * sc))
    nx = np.floor(np.ceil(n / ny))
    #f, sbplts = plt.subplots(int(ny), int(nx), sharex='col', sharey='row')
    f, sbplts = plt.subplots(int(ny), int(nx), sharex=True, sharey=True)
    sbplts = np.array(sbplts).reshape(-1)
    print sbplts.shape
    def _plt(s, i):
        sbplts[i].plot(s[:,0], s[:,1], 'b.')
    dst = src
    _plt(dst, 0)
    for i in xrange(1, n):
        dst = baker_map(dst)
